...
class Container_43c9dc0eca extends Nette\DI\Container
{
	...
	protected array $wiring = [
		...
		'App\UI\Fridge\FridgePresenter' => [2 => ['application.3']],
	...
	public function createServiceApplication__3(): App\UI\Fridge\FridgePresenter
	{
		$service = new App\UI\Fridge\FridgePresenter($this->getService('database.default.explorer'));
		$service->injectPrimary(
			$this->getService('http.request'),
			$this->getService('http.response'),
			$this->getService('application.presenterFactory'),
	...
	public function createServiceApplication__4(): App\UI\Home\HomePresenter
	{
		...
		$service->db = $this->getService('database.default.explorer');
		$service->invalidLinkMode = 5;
		return $service;
	}
	...
	public function createServiceDatabase__default__connection(): Nette\Database\Connection
	{
		$service = new Nette\Database\Connection(
			'mysql:host=database;dbname=fridge',
			/*sensitive{*/'fridge'/*}*/,
			/*sensitive{*/'fridge'/*}*/,
			[],
		);
	...
	public function createServiceLatte__latteFactory(): Nette\Bridges\ApplicationLatte\LatteFactory
	{
		return new class ($this) implements Nette\Bridges\ApplicationLatte\LatteFactory {
			public function __construct(
				private Container_43c9dc0eca $container,
			) {
			}
